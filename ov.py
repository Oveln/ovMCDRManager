...
def LayBot(server, Name, arti):
    Bots[Name][4] = arti
    server.execute('player ' + Name + ' spawn at ' + Bots[Name][0] + ' ' + Bots[Name][1] + ' ' + Bots[Name][
        2] + ' facing 1 1 in ' + ele[Bots[Name][3]])
...
def on_player_left(server, player):
    ...
    if InList(player, WhiteList):
        ...
        # 在线时长更新
        now = Players[player]
        times = datetime.timedelta(days=now[0][0], seconds=now[0][1]) + (
                datetime.datetime.now() - datetime.datetime.strptime(now[1], '%Y-%m-%d-%H:%M:%S'))
